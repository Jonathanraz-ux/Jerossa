...
const OrderDetails = () => {
        ...
        <section className="page-hero" style={{ height: '420px' }}>
          <div className="page-hero-content">
            <nav className="anim-fade-down" style={{ marginBottom: '16px' }}>
              ...
              <li style={{ color: '#fff', fontWeight: 500 }}>{t('order.notFound')}</li>
            </ol>
          </nav>
          <span className="page-hero-surtitre anim-fade-up stagger-1">{t('order.error')}</span>
          <h1 className="page-hero-title anim-fade-up stagger-2">{t('order.notFound')}</h1>
          <p className="page-hero-subtitle anim-fade-up stagger-3">{t('order.notFoundText')}</p>
        </div>
      </section>
      <div className="scroll-animate" style={{ padding: '40px 0' }}>
        <Link to="/my-orders" className="btn btn-primary premium-btn" style={{ padding: '14px 28px', borderRadius: '8px', fontWeight: 600, textDecoration: 'none', color: '#fff', background: 'var(--primary)', border: 'none', cursor: 'pointer', transition: 'all 0.2s' }}>{t('order.backToOrders')}</Link>
      </div>
      </div>
    );
  }
  ...
  const statusLabel = order.statusLabel || t('status.' + order.status) || order.status;
    ...
    <div className="container" style={{ minHeight: '80vh' }}>
        ...
        <div className="page-hero-content">
          <nav className="anim-fade-down" style={{ marginBottom: '16px' }}>
            <ol style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', listStyle: 'none', padding: 0, margin: 0, fontSize: '13px', color: 'rgba(255,255,255,0.7)' }}>
              <li><a href="/" className="link-premium" style={{ color: 'rgba(255,255,255,0.7)' }}>{t('nav.home')}</a></li>
              <li style={{ color: 'rgba(255,255,255,0.4)' }}>/</li>
              <li><Link to="/my-orders" className="link-premium" style={{ color: 'rgba(255,255,255,0.7)' }}>{t('account.myOrders')}</Link></li>
              <li style={{ color: 'rgba(255,255,255,0.4)' }}>/</li>
              <li style={{ color: '#fff', fontWeight: 500 }}>{order.id}</li>
            </ol>
          </nav>
          <span className="page-hero-surtitre anim-fade-up stagger-1">{t('order.order')}</span>
          <h1 className="page-hero-title anim-fade-up stagger-2">{order.id}</h1>
          <p className="page-hero-subtitle anim-fade-up stagger-3">{t('order.orderDate')} {formatDate(order.date, lang)}</p>
        </div>
      </section>
      ...
      <div className="scroll-animate" style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: '32px', flexWrap: 'wrap', gap: '16px' }}>
        <div>
          ...
          <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>{t('order.orderDate')} {formatDate(order.date, lang)}</p>
        </div>
      ...
      <div className="order-tracking scroll-animate premium-card" style={{ marginBottom: '32px', padding: '20px', background: 'var(--bg-cream)', borderRadius: '12px', border: '1px solid var(--border)' }}>
        <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '16px', fontWeight: 600, marginBottom: '16px', color: 'var(--text-dark)' }}>{t('order.tracking')}</h3>
      ...
      <div className="scroll-animate order-info-grid">
        <div className="premium-card" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '12px', padding: '24px' }}>
          <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '16px', fontWeight: 600, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-dark)' }}>
            <MapPin size={18} style={{ color: 'var(--primary)' }} /> {t('order.shipping')}
          </h3>
          {hasRealAddress ? (
            ...
            <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: 1.6 }}>{t('order.noShippingAddress')}</p>
          )}
        </div>
        <div className="premium-card" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '12px', padding: '24px' }}>
          <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '16px', fontWeight: 600, marginBottom: '16px', display: 'flex', alignItems: 'center', gap: '8px', color: 'var(--text-dark)' }}>
            <CreditCard size={18} style={{ color: 'var(--primary)' }} /> {t('order.payment')}
          </h3>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)' }}>
            {order.paymentMethod === 'card' ? t('payment.cardNumber') : order.paymentMethod === 'mobile' ? t('payment.mobileMoney') : order.paymentMethod === 'transfer' ? t('payment.bankTransfer') : t('order.paymentConfirmed')}
          </p>
        </div>
      </div>
      ...
      {order.paymentStatus === 'paid' && ['paid', 'shipped', 'delivered'].includes(order.status) && (
        <div className="scroll-animate" style={{ marginBottom: '32px', padding: '20px 24px', background: 'var(--bg-cream)', borderRadius: '12px', border: '1px solid var(--border)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: '16px' }}>
          <div>
            <h3 style={{ fontFamily: 'var(--font-serif)', fontSize: '16px', fontWeight: 600, marginBottom: '4px', color: 'var(--text-dark)' }}>{t('order.refundTitle')}</h3>
            <p style={{ fontSize: '13px', color: 'var(--text-muted)', margin: 0 }}>{t('order.refundText')}</p>
          </div>
          <Link to={`/refund-request?order=${order.id}`} className="btn btn-outline" style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', padding: '12px 20px', borderRadius: '8px', fontWeight: 600, textDecoration: 'none', border: '1px solid var(--primary)', color: 'var(--primary)', background: 'transparent', cursor: 'pointer', transition: 'all 0.2s' }}>
            <RotateCcw size={16} /> {t('refund.request')}
          </Link>
        </div>
      )}
      ...
      {order.tracking && (
        <div style={{ marginTop: '24px', padding: '16px', background: 'var(--bg-cream)', borderRadius: '8px', display: 'flex', alignItems: 'center', gap: '12px', fontSize: '14px' }}>
          ...
          <span><strong>{t('order.trackingNumber')}:</strong> {order.tracking}</span>
        </div>
      )}
    </div>
  );
