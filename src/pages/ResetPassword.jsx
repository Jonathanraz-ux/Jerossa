        ...
        <div className="page-hero-content">
          <nav className="anim-fade-down" style={{ marginBottom: '16px' }}>
            <ol style={{ display: 'inline-flex', alignItems: 'center', gap: '8px', listStyle: 'none', padding: 0, margin: 0, fontSize: '13px', color: 'rgba(255,255,255,0.7)' }}>
              ...
              <li style={{ color: '#fff', fontWeight: 500 }}>{t('auth.reset.title')}</li>
            </ol>
          </nav>
          <span className="page-hero-surtitre anim-fade-up stagger-1">{t('auth.password')}</span>
          <h1 className="page-hero-title anim-fade-up stagger-2">{t('auth.reset.title')}</h1>
          <p className="page-hero-subtitle anim-fade-up stagger-3">{t('auth.reset.subtitle')}</p>
        </div>
      </section>
          ...
          <div style={{ background: '#fff', borderRadius: '16px', padding: '48px', boxShadow: '0 10px 40px rgba(0, 0, 0, 0.08)', border: '1px solid var(--border)' }}>
            {!session ? (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                ...
                <h3 style={{ fontFamily: 'var(--font-serif)', marginBottom: '8px', color: 'var(--text-dark)' }}>{t('auth.reset.invalid')}</h3>
                <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '24px' }}>
                  {t('auth.reset.invalidText')}
                </p>
                <Link to="/forgot-password" className="btn btn-primary premium-btn" style={{ display: 'inline-block', padding: '14px 28px', borderRadius: '8px', fontWeight: 600, textDecoration: 'none', color: '#fff', background: 'var(--primary)', border: 'none' }}>{t('auth.reset.newLink')}</Link>
              </div>
            ) : success ? (
              <div style={{ textAlign: 'center', padding: '20px 0' }}>
                <CheckCircle size={56} style={{ color: 'var(--success)', marginBottom: '16px' }} />
                <h3 style={{ fontFamily: 'var(--font-serif)', marginBottom: '8px', color: 'var(--text-dark)' }}>{t('auth.reset.updated')}</h3>
                <p style={{ color: 'var(--text-muted)', fontSize: '14px', marginBottom: '24px' }}>
                  {t('auth.reset.updatedText')}
                </p>
                <Link to="/login" className="btn btn-primary premium-btn" style={{ display: 'inline-block', padding: '14px 28px', borderRadius: '8px', fontWeight: 600, textDecoration: 'none', color: '#fff', background: 'var(--primary)', border: 'none' }}>{t('auth.login')}</Link>
              </div>
            ) : (
              <>
                <div style={{ textAlign: 'center', marginBottom: '32px' }}>
                  ...
                  <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: '28px', fontWeight: 600, margin: '0 0 8px', color: 'var(--text-dark)' }}>{t('auth.reset.title')}</h1>
                  <p style={{ color: 'var(--text-muted)', fontSize: '14px' }}>{t('auth.reset.minLength', { n: MIN_PASSWORD_LENGTH })}</p>
                </div>
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="form-group" style={{ marginBottom: '20px' }}>
                    <label className="form-label" style={{ fontSize: '12px', fontWeight: 600, color: 'var(--text-dark)', marginBottom: '6px', display: 'block' }}>{t('auth.reset.newPassword')}</label>
