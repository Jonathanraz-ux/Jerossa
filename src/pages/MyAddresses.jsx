    ...
    <div className="container" style={{ minHeight: '80vh' }}>
      ...
      <section className="page-hero" style={{ height: '420px' }}>
        <div className="page-hero-content">
          ...
          <p className="page-hero-subtitle anim-fade-up stagger-3">{t('account.addressesSubtitle')}</p>
        </div>
      </section>

      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: '32px', flexWrap: 'wrap', gap: '16px' }}>
        <h1 style={{ fontFamily: 'var(--font-serif)', fontSize: '32px', fontWeight: 600, margin: 0, color: 'var(--text-dark)' }}>{t('account.myAddressesTitle')}</h1>
      ...
      <div className="scroll-animate" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: '16px' }}>
        <div className="premium-card" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '12px', padding: '24px', position: 'relative', transition: 'all 0.2s' }}>
          <span className="status-badge" style={{ position: 'absolute', top: '12px', right: '12px', background: 'var(--success-bg)', color: 'var(--success)', padding: '4px 8px', borderRadius: '20px', fontSize: '11px', fontWeight: 600 }}>{t('account.defaultBadge')}</span>
          <h3 style={{ fontWeight: 600, marginBottom: '8px', color: 'var(--text-dark)' }}>{t('account.mainAddress')}</h3>
          ...
          <div style={{ marginTop: '16px', display: 'flex', gap: '8px' }}>
            <button className="btn btn-outline" style={{ fontSize: '12px', padding: '6px 12px', borderRadius: '6px', fontWeight: 500, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', transition: 'all 0.2s' }}>{t('common.edit')}</button>
            <button className="btn btn-outline" style={{ fontSize: '12px', padding: '6px 12px', borderRadius: '6px', fontWeight: 500, border: '1px solid var(--border)', background: 'transparent', cursor: 'pointer', transition: 'all 0.2s', color: 'var(--danger)' }}>{t('common.delete')}</button>
          </div>
        </div>
        ...
        <div className="premium-card" style={{ background: '#fff', border: '1px solid var(--border)', borderRadius: '12px', padding: '24px', transition: 'all 0.2s' }}>
          <h3 style={{ fontWeight: 600, marginBottom: '8px', color: 'var(--text-dark)' }}>{t('account.officeAddress')}</h3>
          <p style={{ fontSize: '14px', color: 'var(--text-muted)', lineHeight: 1.6 }}>
